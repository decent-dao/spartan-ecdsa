...
export class KVCircuitPubInput {
  merkleRoot: bigint;
  Tx: bigint;
  Ty: bigint;
  Ux: bigint;
  ...
  constructor(
    ...
    this.hashPubKeySecret = hashPubKeySecret;
    this.hashSecret = hashSecret;
  }

  serialize(): Uint8Array {
    let serialized = new Uint8Array(32 * 8)
    ...
    serialized.set(bigIntToBytes(this.claimValue, 32), 160);
    serialized.set(bigIntToBytes(this.hashPubKeySecret, 32), 192);
    serialized.set(bigIntToBytes(this.hashSecret, 32), 224);

    return serialized;
  }

  static deserialize(serialized: Uint8Array): KVCircuitPubInput {
    ...
    const claimValue = bytesToBigInt(serialized.slice(160, 192));
    const hashPubKeySecret = bytesToBigInt(serialized.slice(192, 224));
    const hashSecret = bytesToBigInt(serialized.slice(224, 256));

    return new KVCircuitPubInput(merkleRoot, Tx, Ty, Ux, Uy, claimValue, hashPubKeySecret, hashSecret);
  }
}
...
export class PublicInput {
  ...
  rV: bigint;
  msgHash: Buffer;
  circuitPubInput: KVCircuitPubInput;
  ...
  static deserialize(serialized: Uint8Array): PublicInput {
    ...
    const circuitPubInput: KVCircuitPubInput = KVCircuitPubInput.deserialize(
      serialized.slice(32 + 1, 32 + 1 + 32 * 8)
    );
    const msgHash = serialized.slice(32 + 1 + 32 * 8);
